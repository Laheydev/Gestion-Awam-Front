...
export default class Taskbilled extends DynamicCell {
  ...
  showContent(){
    return (<td className="fixedWidth fw-400" name='content' onClick={() => this.setState({editable: true})}>{this.props.content}</td>);
  }
  ...
  sendEdittedValue(event){
    if(event.key === "Enter"){
      this.props.onTimeAdd(event.target.value, moment().format('YYYY-MM-DD'));
